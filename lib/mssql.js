...
function dateToMsSql(val, useUTC) {
  let dateStr, ms;

  if (useUTC){
    dateStr = val.getUTCFullYear() + '-' +
...
MsSQL.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type === Date || prop.type.name === 'Timestamp') {
    ...
    val = dateToMsSql(val, (prop.mssql && prop.mssql.useUTC ? prop.mssql.useUTC : undefined));
    return val;
  }
